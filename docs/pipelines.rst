...
If you do not have access to a computer cluster, but want to run the HTCondor DAGs that can be
generated by the pipelines on a single Linux machine (preferably one with multiple cores and a
...
To allow the Condor jobs to start running immediately rather than waiting for you to stop using you
computer you can create a ``/etc/condor/condor_config.local`` file containing (see the "Test-job
