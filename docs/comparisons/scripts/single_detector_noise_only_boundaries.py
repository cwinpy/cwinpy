...
# set prior for bilby
priors = OrderedDict()
priors['h0'] = Uniform(h0range[0], h0range[1], 'h0', latex_label=r'$h_0$', boundary='reflective')
priors['phi0'] = Uniform(phi0range[0], phi0range[1], 'phi0', latex_label=r'$\phi_0$', unit='rad', boundary='periodic')
priors['psi'] = Uniform(psirange[0], psirange[1], 'psi', latex_label=r'$\psi$', unit='rad', boundary='periodic')
priors['cosiota'] = Uniform(cosiotarange[0], cosiotarange[1], 'cosiota', latex_label=r'$\cos{\iota}$', boundary='reflective')
...
resultdynesty = bilby.run_sampler(
    likelihood=likelihood, priors=priors, sampler='dynesty', nlive=Nlive,
    outdir=outdir, label=label, use_ratio=False)
...
fig = corner.corner(resultdynesty.samples, fig=fig, color='r', bins=50, smooth=0.9,
                    quantiles=[0.16, 0.84],
...
# custom legend
legend_elements = [Line2D([], [], color='r', label='dynesty'),
